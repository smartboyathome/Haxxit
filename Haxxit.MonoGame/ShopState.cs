...
namespace SmartboyDevelopments.Haxxit.MonoGame
{
    class ShopState : HaxxitGameState
    {
        ...
        const int numOfAvailPrograms = 5;

        //buyable Programs
        Haxxit.Tests.DynamicProgramFactory[] mBuyablePrograms;
        ...
        public override void LoadContent(GraphicsDevice graphics, SpriteBatch sprite_batch, ContentManager content)
        {
            ...
            YourProgramsSpriteFont = content.Load<SpriteFont>("Arial14");
            ...
            mBuyablePrograms = new Haxxit.Tests.DynamicProgramFactory[numOfAvailPrograms];
            List<Haxxit.Tests.DynamicDamageCommand> commands = new List<Haxxit.Tests.DynamicDamageCommand>();
            commands.Add(new Haxxit.Tests.DynamicDamageCommand(2, 1, "Glitch"));
            for (int i = 0; i < mBuyablePrograms.Count(); i++)
            {

                //Haxxit.Tests.DynamicDamageCommand command = new Tests.DynamicDamageCommand((ushort) 2, (ushort) 1, "Glitch");
                Haxxit.Tests.DynamicProgramFactory temp = new Haxxit.Tests.DynamicProgramFactory((ushort) (i + 1), (ushort) (i + 2), commands);
                temp.TypeName =  mPlayerProgramImages[i].Name;
                temp.silicoinAmount = (i + 1) * 100;
                mBuyablePrograms[i] = temp;
            }
        }
        ...
        public override void Update()
        {
            ...
            if (isBuyButtonClicked && mAvailSingleProgramSelectedIndex != -1 && mAvailSingleProgramSelectedIndex < mBuyablePrograms.Count())
            {
                if (mouse_state.LeftButton == ButtonState.Released)
                {
                    ...
                    if (mBuyablePrograms[mAvailSingleProgramSelectedIndex].silicoinAmount <= mPlayer1InShop.TotalSilicoins)
                    {
                        if (!mPlayer1InShop.GetPrograms().Contains((ProgramFactory) mBuyablePrograms[mAvailSingleProgramSelectedIndex]))
                        {
                            ...
                            mPlayer1InShop.RemoveSilicoins((ushort)mBuyablePrograms[mAvailSingleProgramSelectedIndex].silicoinAmount);
        ...
        public override void Draw(SpriteBatch sprite_batch)
        {
            ...
            for (int i = 0; i < mBuyablePrograms.Count(); i++)
            {
                ...
                if (mAvailSingleProgramSelectedIndex == i)
                {
                    ...
                    pos.X = AvailProgramsContainerRect.X + AvailProgramsContainerRect.Width - PlayerUISpriteFont.MeasureString(mBuyablePrograms[i].silicoinAmount.ToString()).X - 5;

                    sprite_batch.DrawString(PlayerUISpriteFont, mBuyablePrograms[i].silicoinAmount.ToString(),
                        pos, Color.Black);
                }
                else
                {
